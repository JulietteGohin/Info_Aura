...
class Stats:
    ...
    def hist_height_city(self, postal_code):
        ...
        sub_gdf.hist('bdtopo_bat_hauteur_mean', bins=10, legend = True)
        plt.show()
    ...
    def dpe_city(self, postal_code):
        ...
        plot circular graphic of the DPE in the department
        """
        gdf = gpd.read_file(self.data).groupby(by='code_commune_insee')
...
#A regrouper en une seule fonction avec disjonction de cas selon qu'on veut les dpe du département ou de la ville ?
    ...
    def climatisation_rate(self, postal_code):
        """
        Compute the rate of buildings with air conditioning in the department
        ...
        """
        gdf = gpd.read_file(self.data)
        gdf_clim = gdf[gdf['dpe_logtype_presence_climatisation'] == True]
        rate = len(gdf_clim)/len(gdf)*100

        gdf_city = gdf[gdf['code_commune_insee'] == postal_code]
